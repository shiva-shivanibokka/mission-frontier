  ...
  return (
    <Card className="border-accent-pink/30 p-5">
      <SectionTitle
        icon="⏳"
        title="Catch-up"
        right={<span className="rounded-full bg-accent-pink/15 px-3 py-1 font-mono text-[12px] font-bold text-accent-pink">{total} overdue</span>}
      />
      <p className="mb-3 font-mono text-[12px] text-faint">Scheduled before today and not done yet — clear these to get back on pace.</p>
      <ul className="max-h-80 space-y-2 overflow-y-auto pr-1">
        {overdueLc.map((x) => (
          <li key={x.id} className="flex items-center gap-3 rounded-xl border border-white/8 bg-white/[0.03] px-3 py-2">
            <span className="grid h-4 w-4 place-items-center rounded bg-white/8 text-[10px] text-faint">·</span>
            {x.difficulty && <DiffPill d={x.difficulty} />}
            <a href={lcUrl(x.slug!)} target="_blank" rel="noreferrer" className="text-[13.5px] font-semibold text-subtle hover:text-accent-teal hover:underline">
              {x.label}
            </a>
            <span className="ml-auto font-mono text-[10px] text-faint">{x.date.slice(5)}</span>
          </li>
        ))}
        {overdueOther.map((x) => (
          <li key={x.id} className="flex items-start gap-3 rounded-xl border border-white/8 bg-white/[0.03] px-3 py-2">
            ...
            <div className="min-w-0 flex-1">
              <span className="text-[13.5px] font-semibold text-subtle">{x.label}</span>
