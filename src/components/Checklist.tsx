...
export default function Checklist({
  ...
  return (
    <Card className="flex h-full flex-col p-5">
      <SectionTitle icon={icon} title={title} right={<span className="font-mono text-[13.5px] text-muted">{done}/{items.length}</span>} />
      <div className="mb-3">
        <ProgressBar value={done} total={items.length} color={color} />
      </div>
      {note && <p className="mb-3 text-[14px] leading-relaxed text-muted">{note}</p>}
        ...
        {items.map((i) => (
          <li key={i.id} className="flex items-start gap-2.5 rounded-lg px-1.5 py-1.5 hover:bg-white/[0.03]">
            ...
            <div className="min-w-0 flex-1">
              <span className={`text-[15px] font-semibold ${store.isChecked(i.id) ? 'text-faint line-through' : 'text-subtle'}`}>{i.label}</span>
              {i.detail && <span className="ml-2 font-mono text-[13px] text-faint">{i.detail}</span>}
              {i.res && (
      ...
      {resources && resources.length > 0 && (
        <div className="mt-4 border-t border-white/8 pt-3">
          <div className="mb-2 font-mono text-[13px] uppercase tracking-wide text-faint">Resources</div>
          <div className="flex flex-wrap gap-2">
            {resources.map((r) => (
              <a key={r.url} href={r.url} target="_blank" rel="noreferrer" className="rounded-lg border border-white/10 bg-white/[0.03] px-2.5 py-1 text-[13.5px] font-semibold text-subtle transition hover:border-accent-teal/50 hover:text-accent-teal">
                {r.label} ↗
              </a>
            ))}
          </div>
