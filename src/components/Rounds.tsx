...
export default function Rounds({ store }: { store: Store }) {
  ...
  return (
    <Card className="p-5">
      <SectionTitle icon="🎤" title="Interview Rounds" right={<span className="font-mono text-[13.5px] text-muted">{done}/{ROUNDS.length} ready</span>} />
      <ol className="relative space-y-2 border-l border-white/10 pl-5">
        {ROUNDS.map((r) => (
          <li key={r.id} className="relative">
            <span className="absolute -left-[27px] top-1.5 grid h-4 w-4 place-items-center rounded-full border border-white/20 bg-canvas text-[11px]">
            ...
            <div className="flex items-start gap-2.5 rounded-xl border border-white/8 bg-white/[0.02] px-3 py-2.5">
              ...
              <div>
                <div className="text-[15.5px] font-bold text-ink">{r.title}</div>
                <div className="text-[14px] text-muted">{r.detail}</div>
                <div className="mt-1 font-mono text-[13px] text-accent-violet">Prep: <span className="text-subtle">{r.prep}</span></div>
