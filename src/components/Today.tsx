  ...
  return (
    <Card className="p-5">
      <SectionTitle
        ...
        title="Today’s Quota"
        right={
          <span className={`rounded-full px-3 py-1 font-mono text-[12px] font-bold ${allDone ? 'bg-accent-teal/20 text-accent-teal' : 'bg-white/8 text-muted'}`}>
      ...
      {/* daily LeetCode */}
      <div className="mb-1.5 font-mono text-[11px] font-bold uppercase tracking-wide text-accent-violet">LeetCode · 3/day</div>
        ...
        {day.problems.map((p) => {
          ...
          return (
            <li key={p.slug} className={`flex items-center gap-3 rounded-xl border px-3 py-2.5 transition ${solved ? 'border-accent-teal/40 bg-accent-teal/10' : 'border-white/8 bg-white/[0.03]'}`}>
              <span className={`grid h-5 w-5 place-items-center rounded-md text-[12px] font-black ${solved ? 'bg-accent-teal/25 text-accent-teal' : 'bg-white/8 text-faint'}`}>{solved ? '✓' : '·'}</span>
              <DiffPill d={p.difficulty} />
              <a href={lcUrl(p.slug)} target="_blank" rel="noreferrer" className="text-[14.5px] font-bold text-ink hover:text-accent-teal hover:underline">{p.title}</a>
      ...
      <div className="mt-4 mb-1.5 font-mono text-[11px] font-bold uppercase tracking-wide text-accent-teal">Also today · math &amp; the rest (this week, paced)</div>
      {todayItems.length === 0 ? (
        <p className="rounded-xl border border-white/8 bg-white/[0.02] px-3 py-2.5 font-mono text-[12.5px] text-muted">Nothing else scheduled today — get ahead on this week’s plan below.</p>
      ) : (
        <ul className="space-y-2">
          {todayItems.map((x) => {
            ...
            return (
              <li key={x.id} className="flex items-start gap-3 rounded-xl border border-white/8 bg-white/[0.03] px-3 py-2.5">
                ...
                <div className="min-w-0 flex-1">
                  <span className={`text-[13.5px] font-semibold ${done ? 'text-faint line-through' : 'text-subtle'}`}>{x.label}</span>
                  {x.res && x.res.length > 0 && (
                    <span className="ml-2 inline-flex flex-wrap gap-x-2.5 gap-y-1">
                      {x.res.map((r) => (
                        <a key={r.url} href={r.url} target="_blank" rel="noreferrer" className="whitespace-nowrap font-mono text-[11px] font-bold text-accent-teal/90 hover:text-accent-teal hover:underline">{r.label} ↗</a>
                      ))}
                    </span>
                  )}
                </div>
                <span className="ml-auto shrink-0 rounded-full bg-white/6 px-2 py-0.5 font-mono text-[10px] font-bold text-faint">{x.tag}</span>
              </li>
            )
          })}
        </ul>
