    ...
    <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
      {metrics.map((m) => (
        <Card key={m.label} className="p-4">
          <div className="font-mono text-[11.5px] uppercase tracking-wide text-muted">{m.label}</div>
          <div className="mt-1.5 font-display text-[30px] font-extrabold text-ink">
            {m.value}
            {m.total > 0 && <span className="ml-1 text-[15px] font-bold text-faint">/ {m.total}</span>}
          </div>
          <div className="mb-2.5 font-mono text-[11.5px] text-faint">{m.sub}</div>
