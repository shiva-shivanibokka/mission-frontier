      ...
      {open && (
        <div className="absolute right-0 top-full z-30 mt-2 w-[320px] rounded-xl border border-white/10 bg-card/95 p-3 text-left backdrop-blur-md">
          ...
          <div className="mt-2 flex items-center gap-2">
            <button onClick={save} className="rounded-lg border border-accent-teal/50 bg-accent-teal/15 px-3 py-1 text-[13.5px] font-bold text-accent-teal">Save & sync</button>
            <button onClick={() => { setToken(''); setVal(''); setMsg('cleared') }} className="rounded-lg border border-white/12 px-3 py-1 text-[13.5px] font-bold text-muted">Clear</button>
